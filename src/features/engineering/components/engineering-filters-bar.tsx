...
import {
  countLogsByCategory,
  hasActiveEngineeringFilters,
  type EngineeringFilters,
} from "../lib/filters";

interface EngineeringFiltersBarProps {
...
export function EngineeringFiltersBar({
  categories,
  logs,
  filters,
  resultCount,
}: EngineeringFiltersBarProps) {
  ...
  const updateFilters = useCallback(
    (updates: Partial<EngineeringFilters>) => {
  ...
  const active = hasActiveEngineeringFilters(filters);
  ...
  return (
    <div
      ...
      aria-label="Engineering log filters"
    >
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="relative w-full lg:max-w-md">
          <label htmlFor="engineering-search" className="sr-only">
          ...
          {categories.map((category) => (
            ...
            >
              ...
              <span className="text-muted">
                ({countLogsByCategory(logs, category.slug)})
              </span>
            </FilterChip>
          ))}
        </div>
